...
export class NetworkScanner {
  ...
  async scanSingleHost(ip: string): Promise<Host> {
    ...
    try {
      ...
      // Use -Pn to skip host discovery (we know the host is up)
      // Use --privileged to attempt OS detection if running with sudo
      const nmapCommand = `nmap -p- -sV -O -T4 --osscan-guess -R --system-dns -Pn --version-intensity 9 ${ip} -oX -`;
      this.logger.info(`Executing: ${nmapCommand}`);
